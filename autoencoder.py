...
def calculate_renyi_divergence(mu1, sigma1, alpha):
    #先验分布为标准正态分布
    mu0 = torch.zeros_like(mu1)
    sigma0 = torch.ones_like(sigma1)

    # 计算Renyi 散度
    sigma_alpha_sq = (1 - alpha) + alpha * sigma1**2
    div = (alpha * mu1**2) / (2 * sigma_alpha_sq+EPS) + (1 / (1 - alpha)) * torch.log(abs(sigma_alpha_sq / (sigma1**alpha+EPS)))
    # EPS 是为了避免分母为0 情况出现，abs亦然
    #print(div)
    return div
...
class VGAE(GAE):
    ...
    def kl_loss(self, mu: Optional[Tensor] = None,
        ...
        return -0.5*torch.mean(
             torch.sum( calculate_renyi_divergence(mu, logstd.exp(), 1.05), dim=1))
